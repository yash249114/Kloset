...
export default function Homepage() {
  ...
  const [occasionOutfits, setOccasionOutfits] = useState<Partial<Outfit>[]>([]);
  const [reviews, setReviews] = useState<{ id: string; author: string; text: string; stars: number; date: string }[]>(MOCK_REVIEWS);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const reviewResp = await reviewsAPI.listAll(3);
        if (reviewResp && reviewResp.length > 0) {
          setReviews(reviewResp.map((r) => ({
            id: r.id,
        ...
        console.warn('Could not load reviews, using fallbacks.', err);
      }
    }
    loadData();
  }, []);
    ...
    async function loadOccasion() {
      try {
        ...
        if (resp && resp.outfits.length > 0) {
          ...
          setOccasionOutfits([...MOCK_TRENDING, ...MOCK_NEW].slice(0, 4));
        }
      } catch (err) {
        console.warn('Could not load occasion outfits, using fallbacks.', err);
        setOccasionOutfits([...MOCK_TRENDING, ...MOCK_NEW].slice(0, 4));
      }
    }
    loadOccasion();

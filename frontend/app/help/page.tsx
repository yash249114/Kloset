...
import { Search, ChevronDown, BookOpen, Truck, Shield, Calendar, HelpCircle, FileText, Undo } from 'lucide-react';
...
export default function HelpCenterPage() {
  ...
  const categories = [
    { id: 'all', label: 'All Policies', icon: BookOpen },
    ...
    <div className="min-h-screen relative pb-20" style={{ background: 'var(--ivory)' }}>
      ...
      <div className="relative overflow-hidden py-16 text-center">
        <div className="container mx-auto px-6 relative z-10 space-y-4">
          <FloatIn>
            <span className="text-[10px] font-mono tracking-[0.3em] uppercase text-[var(--rose)] font-bold">
              Kloset Support Desk
            </span>
            ...
            <p className="text-sm text-[var(--ink-light)] max-w-lg mx-auto mt-2">
              Search returns, security deposit refunds, size guides, and platform policies.
            </p>
          </FloatIn>
          ...
          <FloatIn delay={0.1}>
            <div className="max-w-md mx-auto relative mt-6">
              ...
              <input
                type="text"
                placeholder="Search policies (e.g. refund status, return pickup)..."
                value={searchQuery}
                ...
                <button
                  ...
                  className={`flex items-center gap-2 py-2.5 px-5 rounded-2xl text-xs font-mono uppercase tracking-wider transition-all cursor-pointer ${
                    selectedCategory === cat.id
                      ...
                      : 'bg-white border border-[var(--petal)] text-[var(--ink-light)] hover:border-[var(--rose)]'
                  }`}
                >
                  <Icon size={14} />
                  {cat.label}
          ...
          <div className="space-y-4">
            {filteredPolicies.length === 0 ? (
              <div className="empty-floral py-12">
                <p className="text-sm text-[var(--ink-light)]">No policies match your search term.</p>
                <button
                  onClick={() => {
                    setSearchQuery('');
                    setSelectedCategory('all');
                  }}
                  className="text-xs text-[var(--rose)] hover:underline mt-2 font-mono uppercase tracking-wider"
                >
                  Clear filters
                </button>
              </div>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className="w-full p-5 text-left flex justify-between items-center hover:bg-[var(--bloom)]/10 transition-colors cursor-pointer"
                    >
                      ...
                      <ChevronDown
                        ...
                        className={`text-[var(--ink-light)] transition-transform duration-300 ${
                          isCurrentOpen ? 'rotate-180 text-[var(--rose)]' : ''
                        }`}
                      />
                    </button>
        ...
        <FloatIn delay={0.4}>
          ...
          >
            <h3 className="font-display text-lg font-bold text-[var(--ink)]">
              Still have questions about renting?
            </h3>
            <p className="text-xs text-[var(--ink-light)] max-w-md mx-auto">
              Our support team is available 24/7. Chat with our virtual care assistant using the floating widget in the bottom corner, or drop an email to support@kloset.in.
            </p>
            <div className="flex justify-center pt-2">
              <button
                onClick={() => {
                  // Dispatch floating bot event or toggle
                  const el = document.querySelector('button[class*="fixed bottom-6 right-6"]') as HTMLButtonElement;
                  if (el) el.click();
                }}

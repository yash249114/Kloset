...
interface CartStore {
  ...
  updateItemDates: (id: string | number, size: string, startDate: string, endDate: string) => void;
  updateItemSize: (id: string | number, oldSize: string, newSize: string) => void;
  updateItemQuantity: (id: string | number, size: string, quantity: number) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addItem: (newItem) => {
        set((state) => {
          ...
          return {
            ...
            isOpen: true, // Auto open cart drawer when item added
          };
        });
      },
      ...
      getCalculations: () => {
        ...
        const shippingFee = cartItems.length > 0 ? 25 : 0; // Flat shipping rate if cart not empty
